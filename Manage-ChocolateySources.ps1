...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Log "choco source add exited with code $LASTEXITCODE. Output: $addOutput" -Level ERROR
        exit 1
    }
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Log "choco source disable exited with code $LASTEXITCODE. Output: $disableOutput" -Level WARN
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Log "choco upgrade exited with code $LASTEXITCODE. Output: $upgradeOutput" -Level ERROR
        exit 1
    }
