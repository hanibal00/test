...
# ─────────────────────────────────────────────
#  STEP 2 — Disable ALL sources except win-add-source
#           Matched by BOTH name and URL to be safe
# ─────────────────────────────────────────────
Write-Log 'STEP 2: Disabling all Chocolatey sources except "win-add-source" ...'

try {
    # --limitoutput produces stable pipe-delimited lines:
    # name|url|disabled|bypassProxy|selfService|priority
    $sourceLines = & choco source list --limitoutput 2>&1

    if ($LASTEXITCODE -ne 0) {
        Write-Log "choco source list exited with code $LASTEXITCODE. Output: $($sourceLines -join ' ')" -Level ERROR
        exit 1
    }
    ...
    foreach ($line in $sourceLines) {
        ...
        # Need at least name (0) and url (1)
        if ($parts.Count -lt 2) { continue }

        $srcName = $parts[0].Trim()
        $srcUrl  = $parts[1].Trim()

        # Skip the newly added source — match on BOTH name and URL
        if ($srcName -eq $sourceName -and $srcUrl -eq $sourceUrl) {
            Write-Log "Skipping newly added source: '$srcName' ($srcUrl)"
            continue
        }

        $sourcesToDisable += $srcName
    }

    if ($sourcesToDisable.Count -eq 0) {
        Write-Log 'No other sources found to disable.'
    }
    else {
        Write-Log "Found $($sourcesToDisable.Count) source(s) to disable: $($sourcesToDisable -join ', ')"

        foreach ($src in $sourcesToDisable) {
    ...
    Write-Log "Exception while listing sources: $_" -Level ERROR
    exit 1
}
